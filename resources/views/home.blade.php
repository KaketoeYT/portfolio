<x-layout>
    ...
    }" @keydown.right.window="next()" @keydown.down.window="next()" @keydown.left.window="prev()"
        @keydown.up.window="prev()" @wheel.window="handleWheel($event)"
        ...
        <nav class="fixed left-8 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-10">
            @foreach ($sections as $s)
                <button @click="active = '{{ $s['id'] }}'" class="group flex items-center gap-4">
                    <div :class="active === '{{ $s['id'] }}' ? 'bg-indigo-400 w-10' :
                        'bg-slate-800 w-3 group-hover:bg-slate-600'"
