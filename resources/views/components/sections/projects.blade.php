    ...
    <div x-data="{
        ...
        <div class="flex-1 min-w-0">
            @foreach ($projects as $idx => $p)
                <div x-show="current === {{ $idx }}" x-transition:enter="transition ease-out duration-500"
                    x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">

                    <h2
                        class="heading-font text-4xl md:text-6xl text-white mb-4 uppercase italic break-words leading-tight">
                        {{ $p['name'] }}
                    </h2>
                        ...
                        <div class="flex gap-3">
                            <div class="flex flex-wrap gap-2">
                                @foreach ($p['stack'] as $tech)
                                    <span
                                        class="px-2 py-1 bg-indigo-500/5 text-indigo-400/80 border border-indigo-500/20 text-[9px] font-mono tracking-widest uppercase">
                                        {{ $tech }}
                                    </span>
                                @endforeach
                            </div>
        ...
        <div class="w-full md:w-72 flex flex-col gap-3">
            ...
            @foreach ($projects as $idx => $p)
                ...
                <button @click="current = {{ $idx }}"
                    ...
                    @if ($isPortfolio)
                        <div
                            class="absolute top-0 right-0 w-2 h-2 border-t border-r border-indigo-500/40 group-hover:border-indigo-500 transition-colors">
                        </div>
                    @endif
